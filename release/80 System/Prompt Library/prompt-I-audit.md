...
Before calculating scores, adopt a cynical, adversarial stance. Assume problems exist and find at least 5 of them. Look specifically for:
1. Notes that claim to be permanent but read like tasks or ephemeral thoughts.
2. Tags that exist but never connect to anything else.
3. Links that exist but have no real rationale or meaning.
4. Hubs that were created but never actually used as entry points.
5. Permanent notes that contradict each other but nobody noticed.
Find these issues. Zero findings is not acceptable.
...
[List the 5+ concrete problems found during the adversarial pass. Name the specific notes/tags/links.]

## ⚠️ WARNINGS
[List each broader issue with: what it is, specific notes/tags affected, recommended action]
