...
FR1: System must classify every inbox note into exactly one of four categories: DELETE, TASK, SOURCE, or CLAIM (Triage — Prompt J).
FR2: System must verify atomicity of CLAIM notes, producing one of four assessments: ATOMIC, MICRO-ATOMIC, MACRO, or NON-ENDURING (Atomicity Check — Prompt A).
FR3: When a note is MACRO, system must propose concrete atomic splits with specific line ranges, proposed titles, and rationale (Prompt A).
FR4: When a note is MICRO-ATOMIC, system must suggest a parent note to merge into (Prompt A).
FR5: System must assess enduring value of CLAIM notes (transcends context, reference in 6 months, reusable building block) and recommend PROMOTE, INCUBATE, or DISCARD (Prompt A).
...
FR8: System must propose new tags with confidence scores; proposals below 0.60 must be flagged for manual review (Prompt B).
...
FR11: Every proposed link must include the target note, relation verb, one-sentence rationale, and confidence score (Prompt C).
...
FR13: When a contradiction is detected, system must generate a synthesis scaffold with Claim A, Claim B, and possible resolution frameworks — human writes the actual synthesis (Prompt G).
...
FR19: System must run a monthly Luhmann Health Audit calculating atomicity score, connectivity rate, emergence count, cross-domain surprises, output citations, and overall health score (Audit — Prompt I).
...
FR24: AI suggestions must include confidence scores and plain-language reasoning for every recommendation.
...
**Phase 2 — Python Orchestration Layer**
- Python agent (librarian_agent.py) with file-system watching for inbox detection
- SQLite embeddings database for semantic search (sentence-transformers all-MiniLM-L6-v2)
- Automated note processing pipeline with Obsidian Local REST API integration
- Configurable automation thresholds (confidence scores for tagging, linking, hub creation) via config.yml
- Custom rollback scripts for AI decision reversal
- Optional file watcher (opt-in, never default)
...
- Potential hosted option for non-technical users
