...
ZettleLib is an AI orchestration layer that sits on top of Obsidian and enforces genuine Zettelkasten discipline — automatically. It watches your vault, classifies incoming notes, enforces atomicity, prevents tag drift, surfaces non-obvious connections between ideas, and flags contradictions between what you believed before and what you're learning now. The AI does the labor. You make the intellectual decisions. The result is a knowledge system that actually compounds over time — one that teaches you the method while managing the maintenance.
...
ZettleLib is a Python-based agent that runs alongside your Obsidian vault and implements an intelligent, human-governed note processing pipeline.

**Core workflow — when a note enters your inbox, ZettleLib activates:**

1. **Triage** — Classifies every capture as Delete, Task, Source Note, or Claim. Stops collector's fallacy before it starts.
2. **Atomicity Check** — Verifies each permanent note carries exactly one testable claim. Proposes splits when it detects multiple ideas.
3. **Semantic Tagging** — Tags from a maintained canonical taxonomy, flagging synonyms before they become drift.
4. **Link Discovery** — Surfaces connections using semantic relation verbs (*extends, contradicts, analogizes, exemplifies*) — not keyword matches. Every link requires a one-sentence rationale.
5. **Dialectic Detection** — Catches contradictions between new notes and existing ones. Creates synthesis scaffolds for the human to resolve.
6. **Hub Tracking** — Recommends Maps of Content only after actual usage patterns emerge, not prematurely.
7. **Serendipity Engine** — Daily surfacing of non-obvious cross-domain connections you wouldn't find manually.
8. **Vault Health Audit** — Monthly Luhmann-standard health reports: atomicity score, connectivity rate, emergence metrics.
